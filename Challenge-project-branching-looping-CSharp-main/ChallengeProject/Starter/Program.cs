...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "Edad: ?" && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        Console.WriteLine($"La edad de la mascota {ourAnimals[i, 0]}");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalAge = readResult;
                    ...
                    ourAnimals[i, 2] = "Edad: " + animalAge.ToString();
                }

                if (ourAnimals[i, 4] != "Descripción física: " && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        Console.WriteLine($"La descripción física de la mascota {ourAnimals[i, 0]} (tamaño, color, género, peso, entrenamiento)");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalPhysicalDescription = readResult.ToLower();
            ...
            Console.WriteLine("La edad y la descripción fisica se ha completado. Presione Enter para continuar.");
            readResult = Console.ReadLine();

            break;

        case "4":
            // Asegurar que el apodo del animal y la descripción de su personalidad estén completos
            //
            //    ourAnimals[i, 3] = "Apodo: " + animalNickname;
            //    ourAnimals[i, 5] = "Personalidad: " + animalPersonalityDescription;
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 3] == "Apodo: " && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        Console.WriteLine($"Ingrese un apodo para {ourAnimals[i, 0]}:");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalNickname = readResult;
                            entradaValida = animalNickname != "";
                        }
                ...
                if (ourAnimals[i, 5] == "Personalidad: " && ourAnimals[i, 0] != "ID #: ")
                {
                    do
                    {
                        Console.WriteLine($"Ingrese una descripción de la personalidad de {ourAnimals[i, 0]} (gustos o disgustos, trucos, nivel de energía):");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalPersonalityDescription = readResult.ToLower();
                            entradaValida = animalPersonalityDescription != "";
                        }
                    } while (!entradaValida);
            ...
            Console.WriteLine("\n\rLos campos de apodos y descripción de personalidad están completos para todos nuestros amigos. \n\rPresione la tecla Enter para continuar");
            readResult = Console.ReadLine();
